...
function setChecked(interval){
	...
	check.src = chrome.runtime.getURL("./icons/check.png");
...
injectScript(chrome.runtime.getURL('page-context.js'), 'body');
...
function setupMessageBox() {
	let image = '<img src="' + chrome.runtime.getURL('./icons/icon.png') + '" align="left" style="margin-right: 4px;" />';
...
function makeRequest_2captcha(result){
	...
	let url = `https://2captcha.com/in.php?key=${key}&googlekey=${id}&method=userrecaptcha&soft_id=2151&pageurl=${pageurl}&invisible=${isVisible}`;
...
{
	document.getElementsByClassName('grecaptcha-badge').length>0 ? isVisible=1: isVisible=0;
